...
#include "go_rng_cooked.h"

/* ---------- Go math/rand rngSource (go1.14.3 exact port) ---------- */
#define GO_RNGLEN 607
#define GO_RNGTAP 273
#define GO_INT32MAX 2147483647LL
static int32_t go_seedrand(int32_t x){
    int32_t hi=x/44488, lo=x%44488;
    x=48271*lo-3399*hi;
    if(x<0) x+=(int32_t)GO_INT32MAX;
    return x;
}
typedef struct { int tap, feed; int64_t vec[GO_RNGLEN]; } go_rng_t;
static void go_rng_seed(go_rng_t*g, int64_t seed){
    g->tap=0; g->feed=GO_RNGLEN-GO_RNGTAP;
    seed%=GO_INT32MAX; if(seed<0) seed+=GO_INT32MAX; if(seed==0) seed=89482311;
    int32_t x=(int32_t)seed;
    for(int i=-20;i<GO_RNGLEN;i++){
        x=go_seedrand(x);
        if(i>=0){
            int64_t u=(int64_t)x<<40;
            x=go_seedrand(x); u^=(int64_t)x<<20;
            x=go_seedrand(x); u^=(int64_t)x;
            u^=rngCooked[i];
            g->vec[i]=u;
        }
    }
}
static uint64_t go_rng_u64(go_rng_t*g){
    if(--g->tap<0) g->tap+=GO_RNGLEN;
    if(--g->feed<0) g->feed+=GO_RNGLEN;
    int64_t x=g->vec[g->feed]+g->vec[g->tap];
    g->vec[g->feed]=x;
    return (uint64_t)x;
}
static int64_t go_rng_i63(go_rng_t*g){ return (int64_t)(go_rng_u64(g)&0x7fffffffffffffffULL); }
...
typedef struct {
    ...
    go_rng_t go;           // go math/rand rngSource
    int64_t gval; int gpos; // go Read() leftover (7 bytes per Int63)
} prng_t;
...
static void prng_init(prng_t*p, int scheme, uint32_t seed){
    ...
    switch(scheme){
        ...
        // word-picking schemes (raw draws, 11-bit indices)
        case 30: case 31: mt_init(p,seed); break;                        // MT19937 low11 / high11
        case 32: p->js=(seed^0x5DEECE66Du)&((1ULL<<48)-1); break;        // Java nextInt(2048)
        case 33: case 34: p->xs=(seed?seed:0x9E3779B9u); break;          // xorshift32 low11 / high11
        case 35: ginit(p,seed); break;                                   // glibc grand & 2047
        case 36: p->ls=seed; break;                                      // LCG1664525 high11
        case 40: case 41: case 42: go_rng_seed(&p->go,(int64_t)seed); p->gpos=0; break; // Go math/rand
    }
}

/* raw 11-bit index draw for word-picking schemes */
static uint32_t prng_idx11(prng_t*p){
    switch(p->scheme){
        case 30: return mt_u32(p)&2047;
        case 31: return mt_u32(p)>>21;
        case 32: { p->js=(p->js*0x5DEECE66Du+0xBu)&((1ULL<<48)-1); return (uint32_t)(p->js>>37); } // next(11)
        case 33: { p->xs^=p->xs<<13; p->xs^=p->xs>>17; p->xs^=p->xs<<5; return p->xs&2047; }
        case 34: { p->xs^=p->xs<<13; p->xs^=p->xs>>17; p->xs^=p->xs<<5; return p->xs>>21; }
        case 35: return (uint32_t)grand(p)&2047;
        case 36: { p->ls=p->ls*1664525u+1013904223u; return (uint32_t)(p->ls>>21)&2047; }
        case 41: return (uint32_t)(go_rng_i63(&p->go)>>32)&2047;         // Go Intn(2048) = Int31()&(2047)
        case 42: return (uint32_t)(go_rng_i63(&p->go)&2047);             // Go Int63()%2048
    }
    return 0;
}
...
static uint8_t prng_byte(prng_t*p){
    if(p->scheme==40){ // Go Rand.Read: 7 bytes LE per Int63
        if(p->gpos==0){ p->gval=go_rng_i63(&p->go); p->gpos=7; }
        uint8_t b=(uint8_t)(p->gval&0xff); p->gval>>=8; p->gpos--;
        return b;
    }
    switch(p->scheme){
...
int main(int argc,char**argv){
    ...
    for(uint32_t s=s0; s<s1; s++){
        ...
        if(scheme>=30 && scheme!=40){
            mn[0]=0;
            for(int w=0; w<12; w++){
                strcat(mn, wordlist[prng_idx11(&p)]);
                if(w<11) strcat(mn," ");
            }
            ent[0]=0; // unused in word-picking mode
        } else {
